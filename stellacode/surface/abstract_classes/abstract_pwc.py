...
class PWC_Surface(Surface):
    ...
    def compute_points(self):
        ...
        points[..., 2] = np.tan(self.beta) * points[::, ::, 1] + np.sin(thetagrid) * self.r(thetagrid) / np.cos(
            self.beta
        )
        ...
        if self.n_cyl == 1:
            ...
            if self.symmetry:
                ...
                for _ in range(self.n_cyl // 2 - 1):
                    np.einsum("ij,uvj->uvi", rotation_matrix, two_cylinders, out=two_cylinders)
                    res = np.concatenate((res, two_cylinders), axis=1)
    ...
    def compute_first_derivatives(self):
        ...
        dydphi = (1 + np.tan(phigrid) ** 2) * self.__P[::, : self.l_v // self.n_cyl :, 0] + np.tan(phigrid) * dxdphi
        ...
        if self.n_cyl == 1:
            ...
            np.einsum("ik,kjuv->ijuv", sym_mat_2T, jacobianT_sym, out=jacobianT_sym)
            np.einsum("ikuv,kj->ijuv", jacobianT_sym, sym_mat_1T, out=jacobianT_sym)

            # Transposed Jacobian of the first two cylinders
            two_cylinders_jacobianT = np.concatenate((jacobianT, jacobianT_sym), axis=3)
            ...
            if self.symmetry:
                ...
                for _ in range(self.n_cyl // 2 - 1):
                    ...
                    np.einsum(
                        "ik,kjuv->ijuv",
                        sym_mat_2T,
                        two_cylinders_jacobianT[..., ::-1],
                        out=two_cylinders_jacobianT,
                    )
                    np.einsum(
                        "ikuv,kj->ijuv",
                        two_cylinders_jacobianT,
                        symmetry_matrixT,
                        out=two_cylinders_jacobianT,
                    )

                    full_jacobianT = np.concatenate((full_jacobianT, two_cylinders_jacobianT), axis=3)
    ...
    def compute_second_derivatives(self):
        ...
        d2xdtheta2 = (
            ...
            / np.sin(phigrid + self.alpha)
            * (
                np.cos(thetagrid) * self.r_second(thetagrid)
                - 2 * np.sin(thetagrid) * self.r_prime(thetagrid)
        ...
        d2xdphi2 = (
            ...
            * np.cos(self.alpha)
            * (self.R0 * np.sin(self.alpha) + np.cos(thetagrid) * self.r(thetagrid))
            / (np.tan(phigrid + self.alpha) * np.sin(phigrid + self.alpha) ** 2)
        )
        ...
        d2ydphi2 = (
            np.tan(phigrid) * d2xdphi2
            + 2 * (1 + np.tan(phigrid) ** 2) * self.dpsi[1, 0, :, : self.l_v // self.n_cyl :] / (2 * PI / self.n_fp)
            + 2 * np.tan(phigrid) * (1 + np.tan(phigrid) ** 2) * self.P[::, : self.l_v // self.n_cyl :, 0]
        )

        d2ydv2 = d2ydphi2 * (2 * PI / self.n_fp) ** 2

        d2ydthetadphi = np.tan(phigrid) * d2xdthetadphi + (1 + np.tan(phigrid) ** 2) * self.dpsi[
            0, 0, :, : self.l_v // self.n_cyl :
        ] / (2 * PI)

        d2ydudv = d2ydthetadphi * 2 * PI * 2 * PI / self.n_fp

        d2zdtheta2 = np.tan(self.beta) * d2ydtheta2 + (
            np.sin(thetagrid) * self.r_second(thetagrid)
            + 2 * np.cos(thetagrid) * self.r_prime(thetagrid)
            - np.sin(thetagrid) * self.r(thetagrid)
        ) / np.cos(self.beta)
        ...
        if self.n_cyl == 1:
            ...
            if self.symmetry:
                ...
                for _ in range(self.n_cyl // 2 - 1):
                    ...
                    np.einsum("ij,juv->iuv", sym_mat, d2Psi0du2[..., ::-1], out=d2Psi0du2)
                    np.einsum("ij,juv->iuv", sym_mat, d2Psi0dv2[..., ::-1], out=d2Psi0dv2)
                    np.einsum("ij,juv->iuv", -sym_mat, d2Psi0dudv[..., ::-1], out=d2Psi0dudv)

                    fulld2psidu2 = np.concatenate((fulld2psidu2, d2Psi0du2), axis=2)
                    fulld2psidv2 = np.concatenate((fulld2psidv2, d2Psi0dv2), axis=2)
                    fulld2psidudv = np.concatenate((fulld2psidudv, d2Psi0dudv), axis=2)
        ...
        dNdu = np.cross(self.__dpsi_uu, self.dpsi[1], 0, 0, 0) + np.cross(self.dpsi[0], self.__dpsi_uv, 0, 0, 0)
        dNdv = np.cross(self.__dpsi_uv, self.dpsi[1], 0, 0, 0) + np.cross(self.dpsi[0], self.__dpsi_vv, 0, 0, 0)
