...
class Surface_PWC_Fourier(PWC_Surface):
    ...
    def load_file(cls, path_surf, n_fp, n_pol, n_tor):
        ...
        if file_extension == "txt":
            ...
            param = np.concatenate((cos, sin, [R0, alpha, beta]))
    ...
    def __compute_r(self):
        ...
        def r(theta):
            ...
            return np.sum(
                self.fourier_coeffs * np.concatenate((np.cos(theta * np.arange(n)), np.sin(theta * np.arange(n))))
            )
    ...
    def __compute_r_prime(self):
        def r_prime(theta):
            ...
            coeffs = np.concatenate(
                (
                    self.fourier_coeffs[1 : len(self.fourier_coeffs) // 2 :],
                    self.fourier_coeffs[1 + len(self.fourier_coeffs) // 2 : :],
                )
            )
            return np.sum(
                ...
                * np.concatenate(
                    (
                        -np.arange(1, n) * np.sin(theta * np.arange(1, n)),
                        np.arange(1, n) * np.cos(theta * np.arange(1, n)),
                    )
                )
            )
    ...
    def __compute_r_second(self):
        def r_second(theta):
            ...
            return np.sum(
                ...
                * np.concatenate(
                    (
                        -np.arange(1, n) ** 2 * np.cos(theta * np.arange(1, n)),
                        -np.arange(1, n) ** 2 * np.sin(theta * np.arange(1, n)),
                    )
                )
            )
    ...
    def get_theta_pertubation(self, compute_curvature=True):
        ...
        n = n_coeffs // 2
        perturbation = np.empty((n_coeffs + 3, *ugrid.shape, 3), dtype=float_type)

        for k in range(n):
            perturbation[k, :, :, 0] = (
                np.cos(phigrid) / np.sin(phigrid + self.alpha) * np.cos(thetagrid) * np.cos(k * thetagrid)
            )
            perturbation[k, :, :, 1] = np.tan(phigrid) * perturbation[k, :, :, 0]
            perturbation[k, :, :, 2] = np.tan(self.beta) * perturbation[k, :, :, 1] + np.sin(thetagrid) * np.cos(
                k * thetagrid
            ) / np.cos(self.beta)
            perturbation[k + n, :, :, 0] = np.tan(k * thetagrid) * perturbation[k, :, :, 0]
            perturbation[k + n, :, :, 1] = np.tan(k * thetagrid) * perturbation[k, :, :, 1]
            perturbation[k + n, :, :, 2] = np.tan(k * thetagrid) * perturbation[k, :, :, 2]
        ...
        dperturbation = np.empty((n_coeffs + 3, *ugrid.shape, 2, 3))

        for k in range(n):
            # derivatives / phi
            dperturbation[k, :, :, 1, 0] = (
                -np.cos(self.alpha) / np.sin(phigrid + self.alpha) ** 2 * np.cos(thetagrid) * np.cos(k * thetagrid)
            )
            dperturbation[k, :, :, 1, 1] = (1 + np.tan(phigrid) ** 2) * perturbation[k, :, :, 0] + np.tan(
                phigrid
            ) * dperturbation[k, :, :, 1, 0]
            dperturbation[k, :, :, 1, 2] = np.tan(self.beta) * dperturbation[k, :, :, 1, 1]

            dperturbation[k + n, :, :, 1, 0] = np.tan(k * thetagrid) * dperturbation[k, :, :, 1, 0]
            dperturbation[k + n, :, :, 1, 1] = np.tan(k * thetagrid) * dperturbation[k, :, :, 1, 1]
            dperturbation[k + n, :, :, 1, 2] = np.tan(k * thetagrid) * dperturbation[k, :, :, 1, 2]

            # derivatives / theta
            dperturbation[k, :, :, 0, 0] = (
                -np.cos(phigrid)
                / np.sin(phigrid + self.alpha)
                * (np.sin(thetagrid) * np.cos(k * thetagrid) + k * np.cos(thetagrid) * np.sin(k * thetagrid))
            )
            dperturbation[k, :, :, 0, 1] = np.tan(phigrid) * dperturbation[k, :, :, 0, 0]
            dperturbation[k, :, :, 0, 2] = np.tan(self.beta) * dperturbation[k, :, :, 0, 1] + 1 / np.cos(self.beta) * (
                np.cos(thetagrid) * np.cos(k * thetagrid) - k * np.sin(thetagrid) * np.sin(k * thetagrid)
            )

            dperturbation[k + n, :, :, 0, 0] = (
                np.tan(k * thetagrid) * dperturbation[k, :, :, 0, 0]
                + k * (1 + np.tan(k * thetagrid) ** 2) * perturbation[k, :, :, 0]
            )
            dperturbation[k + n, :, :, 0, 1] = (
                np.tan(k * thetagrid) * dperturbation[k, :, :, 0, 1]
                + k * (1 + np.tan(k * thetagrid) ** 2) * perturbation[k, :, :, 1]
            )
            dperturbation[k + n, :, :, 0, 2] = (
                np.tan(k * thetagrid) * dperturbation[k, :, :, 0, 2]
                + k * (1 + np.tan(k * thetagrid) ** 2) * perturbation[k, :, :, 2]
            )
