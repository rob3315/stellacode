...
class AbstractSurfaceModule(eqx.Module):
    ...
    def compute_surface_attributes(self, grids, deg: int = 2):
        ...
        surf = Surface(grids=grids, xyz=xyz)
        # We also compute surface element dS and derivatives dS_u and dS_v:
        if deg >= 1:
            surf.jac_xyz = self.get_jac_xyz_on_grid(uv_grid)
        ...
        if deg >= 2:
            surf.hess_xyz = self.get_hess_xyz_on_grid(uv_grid)
