...
def MGrid_from_surf(rmin, rmax, zmin, zmax, nfp, rad, zee, phi, myOutput, surf: AbstractSurface, j):
    ...
    def best_discretization(rad, zee, phi, max_mem_array, mem_one_point):
        ...
        nrad_nzee_nphi = model.NewIntVar(0, (rad - 1) * (zee - 1) * (phi - 1), "xyz")
    ...
    PZRgrid[:, :, :, 0], PZRgrid[:, :, :, 1], PZRgrid[:, :, :, 2] = np.meshgrid(phis, zs, rs, indexing="ij")
    ...
    try:
        T = (
            ...
            - contract("opq,ijq->oijp", rot_tensor, surf.P)[:, :, :, np.newaxis, np.newaxis, np.newaxis, :]
        )

        K = T / (np.linalg.norm(T, axis=-1) ** 3)[..., np.newaxis]

        XYZBs = (
            mu_0 / (4 * np.pi) * contract("niq,uvq,nuvpzrj,ijc,uv->pzrc", rot_tensor, j, K, eijk, surf.dS) / surf.npts
        )
        ...
        nrad, nzee, nphi = best_discretization(rad, zee, phi, max_mem_array, mem_one_point)

        print(f"Best discretization found :\n{nrad} r points\n{nzee} z points\n{nphi} phi points")
        ...
        while irad <= rad:
            while izee <= zee:
                while iphi <= phi:
                    ...
                    XYZBs[iphi : iphi + nphi, izee : izee + nzee, irad : irad + nrad, :] = (
                        mu_0
                        / (4 * np.pi)
                        * contract(
                            "niq,uvq,nuvpzrj,ijc,uv->pzrc",
                            rot_tensor,
                            j,
                            K,
                            eijk,
    ...
    mgridio.RZPhiGrid = urf.Volume(
        rep,
        data={
            "Begin": [0.0, zmin, rmin],
            "End": [2 * np.pi / nfp, zmax, rmax],
            "nCells": [phi - 1, zee - 1, rad - 1],
        },
    )
    mgridio.magFields = [
        urf.Field(
...
def MGrid_from_surf_dask(rmin, rmax, zmin, zmax, nfp, rad, zee, phi, myOutput, surf: AbstractSurface, j):
...
def MGrid_from_config(
    ...
    myOutput,
    path_config_file,
    cws: AbstractSurface = None,
    plasma: AbstractSurface = None,
    dask=True,
):
    ...
    if dask:
        MGrid_from_surf_dask(rmin, rmax, zmin, zmax, plasma.n_fp, rad, zee, phi, myOutput, cws, j_3D)
    else:
        MGrid_from_surf(rmin, rmax, zmin, zmax, plasma.n_fp, rad, zee, phi, myOutput, cws, j_3D)
