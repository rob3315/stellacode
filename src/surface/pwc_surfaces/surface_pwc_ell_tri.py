...
class Surface_PWC_Ell_Tri(PWC_Surface):
    ...
    def __compute_r(self):
        ...
        """
        def r(theta):
            return self.__a * np.sqrt((1 + self.__delta * np.cos(theta))**2 * np.cos(theta)**2 + self.__kappa**2 * np.sin(theta)**2)
        self.__r = np.vectorize(r)

    def __compute_r_prime(self):
        def r_prime(theta):
            return self.__a**2 * np.sin(2*theta) * (self.__kappa**2 - (1 + self.__delta * np.cos(theta)) * (1 + 2 * self.__delta * np.cos(theta))) / (2 * self.__r(theta))
        self.__r_prime = np.vectorize(r_prime)

    def __compute_r_second(self):
        def r_second(theta):
            u = np.sin(2*theta) * (self.__kappa**2 - (1 + self.__delta *
                                                      np.cos(theta)) * (1 + 2 * self.__delta * np.cos(theta)))
            u_prime = 2 * np.cos(2*theta) * (self.__kappa**2 - (1 + self.__delta * np.cos(theta)) * (1 + 2 * self.__delta * np.cos(
                theta))) + self.__delta * np.sin(theta) * np.sin(2*theta) * (3 + 4 * self.__delta * np.cos(theta))
    ...
    def get_theta_pertubation(self, compute_curvature=True):
        ...
        perturbation = np.empty(
            (len(self.__param), *ugrid.shape, 3), dtype=float_type)

        # dx / da
        perturbation[0, :, :, 0] = np.cos(phigrid) * np.cos(thetagrid) * self.__r(
            thetagrid) / (self.__a * np.sin(phigrid + self.__alpha))
        # dy / da
        perturbation[0, :, :, 1] = np.tan(phigrid) * perturbation[0, :, :, 0]
        # dz / da
        perturbation[0, :, :, 2] = np.tan(self.__beta) * perturbation[0, :, :, 1] + np.sin(
            thetagrid) * self.__r(thetagrid) / (self.__a * np.cos(self.__beta))

        # dx / dkappa
        perturbation[1, :, :, 0] = self.__a**2 * self.__kappa * np.sin(thetagrid)**2 * np.cos(
            phigrid) * np.cos(thetagrid) / (self.__r(thetagrid) * np.sin(phigrid + self.__alpha))
        # dy / dkappa
        perturbation[1, :, :, 1] = np.tan(phigrid) * perturbation[1, :, :, 0]
        # dz / dkappa
        perturbation[1, :, :, 2] = np.tan(self.__beta) * perturbation[1, :, :, 1] + np.sin(
            thetagrid)**3 * self.__a**2 * self.__kappa / (self.__r(thetagrid) * np.cos(self.__beta))

        # dx / ddelta
        perturbation[2, :, :, 0] = self.__a**2 * np.cos(phigrid) * np.cos(thetagrid)**4 * (
            1 + self.__delta * np.cos(thetagrid)) / (self.__r(thetagrid) * np.sin(phigrid + self.__alpha))
        # dy / ddelta
        perturbation[2, :, :, 1] = np.tan(phigrid) * perturbation[2, :, :, 0]
        # dz / ddelta
        perturbation[2, :, :, 2] = np.tan(self.__beta) * perturbation[2, :, :, 1] + self.__a**2 * np.sin(
            thetagrid) * np.cos(thetagrid)**3 * (1 + self.__delta * np.cos(thetagrid)) / (self.__r(thetagrid) * np.cos(self.__beta))
        ...
        dperturbation = np.empty(
            (len(self.__param), *ugrid.shape, 2, 3))

        # d²x / dphi da
        dperturbation[0, :, :, 1, 0] = - np.cos(self.__alpha) * np.cos(
            thetagrid) * self.__r(thetagrid) / (self.__a * np.sin(phigrid + self.__alpha)**2)
        # d²y / dphi da
        dperturbation[0, :, :, 1, 1] = (
            1 + np.tan(phigrid)**2) * perturbation[0, :, :, 0] + np.tan(phigrid) * dperturbation[0, :, :, 1, 0]
        # d²z / dphi da
        dperturbation[0, :, :, 1, 2] = np.tan(
            self.beta) * dperturbation[0, :, :, 1, 1]

        # d²x / dtheta da
        dperturbation[0, :, :, 0, 0] = np.cos(phigrid) * (self.__r_prime(thetagrid) * np.cos(
            thetagrid) - self.__r(thetagrid) * np.sin(thetagrid)) / (self.__a * np.sin(phigrid + self.__alpha))
        # d²y / dtheta da
        dperturbation[0, :, :, 0, 1] = np.tan(
            phigrid) * dperturbation[0, :, :, 0, 0]
        # d²z / dtheta da
        dperturbation[0, :, :, 0, 2] = np.tan(self.__beta) * dperturbation[0, :, :, 0, 1] + (self.__r(thetagrid) * np.cos(
            thetagrid) + self.__r_prime(thetagrid) * np.sin(thetagrid)) / (self.__a * np.cos(self.__beta))

        # d²x / dphi dkappa
        dperturbation[1, :, :, 1, 0] = - self.__a**2 * np.cos(self.__alpha) * np.cos(
            thetagrid) * self.__kappa * np.sin(thetagrid)**2 / (self.__r(thetagrid) * np.sin(phigrid + self.__alpha)**2)
        # d²y / dphi dkappa
        dperturbation[1, :, :, 1, 1] = (
            1 + np.tan(phigrid)**2) * perturbation[1, :, :, 0] + np.tan(phigrid) * dperturbation[1, :, :, 1, 0]
        # d²z / dphi dkappa
        dperturbation[1, :, :, 1, 2] = np.tan(
            self.beta) * dperturbation[1, :, :, 1, 1]

        # d²x / dtheta dkappa
        dperturbation[1, :, :, 0, 0] = self.__a**2 * self.__kappa * np.cos(phigrid) * np.sin(thetagrid) * (self.__r(thetagrid) * (3 * np.cos(
            thetagrid)**2 - 1) - np.cos(thetagrid) * np.sin(thetagrid) * self.__r_prime(thetagrid)) / (self.__r(thetagrid)**2 * np.sin(phigrid + self.__alpha))
        # d²y / dtheta dkappa
        dperturbation[1, :, :, 0, 1] = np.tan(
            phigrid) * dperturbation[1, :, :, 0, 0]
        # d²z / dtheta dkappa
        dperturbation[1, :, :, 0, 2] = np.tan(self.__beta) * dperturbation[1, :, :, 0, 1] + self.__a**2 * self.__kappa * np.sin(thetagrid)**2 * (
            3 * np.cos(thetagrid) * self.__r(thetagrid) - np.sin(thetagrid) * self.__r_prime(thetagrid)) / (np.cos(self.__beta) * self.__r(thetagrid)**2)

        # d²x / dphi ddelta
        dperturbation[2, :, :, 1, 0] = - np.cos(self.__alpha) * self.__a**2 * np.cos(
            thetagrid)**4 * (1 + self.__delta * np.cos(thetagrid)) / (self.__r(thetagrid) * np.sin(phigrid + self.__alpha)**2)
        # d²y / dphi ddelta
        dperturbation[2, :, :, 1, 1] = (
            1 + np.tan(phigrid)**2) * perturbation[2, :, :, 0] + np.tan(phigrid) * dperturbation[2, :, :, 1, 0]
        # d²z / dphi ddelta
        dperturbation[2, :, :, 1, 2] = np.tan(
            self.__beta) * dperturbation[2, :, :, 1, 1]

        # d²x / dtheta ddelta
        dperturbation[2, :, :, 0, 0] = - self.__a**2 * np.cos(phigrid) * (self.__r(thetagrid) * np.sin(thetagrid) * np.cos(thetagrid)**3 * (4 + 5*self.__delta*np.cos(
            thetagrid)) + self.__r_prime(thetagrid) * np.cos(thetagrid)**4 * (1 + self.__delta * np.cos(thetagrid))) / (np.sin(phigrid + self.__alpha) * self.__r(thetagrid)**2)
        # d²y / dtheta ddelta
        dperturbation[2, :, :, 0, 1] = np.tan(
            phigrid) * dperturbation[2, :, :, 0, 0]
        # d²z / dtheta ddelta
        dperturbation[2, :, :, 0, 2] = np.tan(self.__beta) * dperturbation[2, :, :, 0, 1] + self.__a**2 / (np.cos(self.__beta) * self.__r(thetagrid)**2) * (self.__r(thetagrid) * (5*self.__delta*np.cos(
            thetagrid)**5 + 4*np.cos(thetagrid)**4 - 4*self.__delta*np.cos(thetagrid)**3 - 3*np.cos(thetagrid)**2) - self.__r_prime(thetagrid) * np.sin(thetagrid) * np.cos(thetagrid)**3 * (1 + self.__delta * np.cos(thetagrid)))
