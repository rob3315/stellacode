...
class Constraint(BaseModel):
    ...
    def constraint(self, val):
        """
        Return the delta between a value and the limit :
        - value minus the limit if it is a minimum ;
        - limit minus the value if it is a maximum.
        """
        if self.minimum:
    ...
    def barrier(self, val):
        """
        Return a cost :
        - quadratic method : squared normalized delta if the constraint is violated;
        - inverse method : infinity if the constraint is violated (negative delta),
        cost penalizing the relative distance between the delta and the caracteristic length otherwise;
        - quadratic_log method : infinity if the constraint is violated (negative delta),
        cost penalizing a normalized delta close to 0.
        """
