...
from stellacode.tools import biot_et_savart_op
...
class CurrentCtrCost(AbstractCost):
    ...
    constraint: Constraint = Constraint(
        limit=0.0, distance=1.0, weight=1.0, minimum=False)
...
class PoloidalCurrentCost(AbstractCost):
    ...
    def cost(self, S, results: Results = Results()):
        if isinstance(S, CoilSurface):
            ju = S.j_surface[:, :, 0]
        elif isinstance(S, CoilOperator):
            coil_surf = S.get_coil(phi_mn=results.phi_mn_wnet_cur)
            ju = coil_surf.j_surface[:, :, 0]
        else:
            NotImplementedError

        loss = self.constraint.barrier(ju).sum()
...
class CriticalCurrentCtr(AbstractCost):
    ...
    def cost(self, S, results: Results = Results()):
        ...
        theta = np.arctan2(j_3d / np.norm(j_3d, axis=-1),
                           b_field / np.norm(b_field, axis=-1))
        jc = self.critical_current_tape(
            b_field, theta, temperature=self.temperature) * self.num_hts_turn
