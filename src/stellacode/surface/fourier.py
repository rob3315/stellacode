...
class FourierSurfaceFactory(AbstractSurfaceFactory):
    ...
    def from_file(
        ...
        """
        Create a FourierSurfaceFactory object from a file.
        ...
        Args:
            path_surf (str): Path to the file containing the Fourier coefficients
            integration_par (IntegrationParams): Integration parameters
            nfp (int): Number of field periods. Default is 1.
            surface_label (int): Surface label. Default is -1.

        Returns:
            FourierSurfaceFactory: FourierSurfaceFactory object generated from the file
        """
        ...
        if path_surf[-3::] == ".nc":
            # Load data from VMEC wout file
            vmec = VMECIO.from_grid(path_surf)
            nfp = vmec.nfp
            m = vmec.get_var("xm", int)
            n = -vmec.get_var("xn", int) / nfp
            ...
            # Load data from regcoil nescin file
        ...
        # Create FourierSurfaceFactory object and return it
        return cls(
            Rmn=Rmn,
            Zmn=Zmn,
            mf=m,
    ...
    def get_xyz(self, uv):
        ...
        # Calculate the radial and vertical components of the basis functions
        R = np.tensordot(self.Rmn, np.cos(angle), 1)
        Z = np.tensordot(self.Zmn, np.sin(angle), 1)

        # Calculate the azimuthal angle of the basis functions
        ...
        # Stack the radial, azimuthal, and vertical components along the last axis
...
class FourierSurface(Surface):
    ...
    def get_envelope_fourier_coeff(
        ...
        **kwargs,
        ...
        """
        Compute Fourier coefficients of the envelope of the surface.

        Args:
            num_cyl (int, optional): Number of cylinders if pwc. Defaults to None.
            num_coeff (int, optional): Number of Fourier coefficients. Defaults to 5.
            convex (bool, optional): Whether compute a convex or concave hull. Defaults to False.
            angle (float, optional): Angle of the poloidal plane. Defaults to 0.0.

        Returns:
            numpy.ndarray: Fourier coefficients of the envelope.
        """
        ...
        # Define a function to compute r(theta)
        def fun(theta):
            """Compute r(theta) from interpolated data."""
            return to_polar(*interp(theta))[0]

        # Compute Fourier coefficients
        return fourier_coefficients(th.min(), th.min() + 2 * np.pi,
                                    num_coeff, fun, **kwargs)
    ...
    def get_surface_envelope(
        ...
        **kwargs,
        ...
        """
        a0_2, coefs = self.get_envelope_fourier_coeff(
