...
class Current(AbstractCurrent):
    ...
    def get_grad_current_op(self, grids, max_val_v: float = 1):
        ...
        """
        return np.stack(
            (self(grids, max_val_v=max_val_v, grad="u"),
             self(grids, max_val_v=max_val_v, grad="v")),
            axis=-1,
        )
    ...
    def __call__(self, grids, max_val_v: float = 1, grad: tp.Optional[str] = None):
        ...
        if self.sin_basis:
            ...
            if grad == "u":
                ...
                dphi.append(
                    _stack(2 * onp.pi * xm * xn * cosu * cosv / 2 /
                           max_val_v, 2 * onp.pi * xm**2 * sinu * sinv)
                )
            elif grad == "v":
                # current for Phi=sin(2*onp.pi*xn*v)
                ...
                # current for Phi=sin(2*onp.pi*xm*u)*sin(onp.pi*xn*v)
                dphi.append(_stack(-onp.pi * xn**2 * sinu * sinv /
                            2 / max_val_v, -onp.pi * xn * xm * cosu * cosv))
            elif grad is None:
        ...
        if self.cos_basis:
            if grad == "u":
                dphi.append(
                    _stack(-2 * onp.pi * xm * xn * sinu * cosv / 2 /
                           max_val_v, 2 * onp.pi * xm**2 * cosu * sinv)
                )
            elif grad == "v":
                dphi.append(_stack(-onp.pi * xn**2 * cosu * sinv /
                            2 / max_val_v, onp.pi * xn * xm * sinu * cosv))
            elif grad is None:
