...
def get_cws(config, build_coils: bool = False):
    ...
    coil_factory = CoilFactory(
        current=get_current_potential(config), build_coils=build_coils)
    surface_factory = Sequential(
        surface_factories=[cws, coil_factory, rot_nfp])
...
def get_plasma_surface(config):
    ...
    plasma = FourierSurfaceFactory.from_file(
        path_plasma,
        integration_par=IntegrationParams(
            num_points_u=n_pol_plasma, num_points_v=n_tor_plasma),
        n_fp=int(config["geometry"]["Np"]),
    )
    return plasma
