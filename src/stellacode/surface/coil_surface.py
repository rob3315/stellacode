...
class GroovedCoilFactory(AbstractBaseFactory):
    ...
    Args:
        * u_ctrl_points: poloidal position of the control points of the grooves with dimensions: N_groove + 2 x N_ctr_points
        * v_ctrl_points_w: toroidal position weights of the control points of the grooves with dimensions: N_groove x N_ctr_points
    ...
    u_ctrl_points: Array
    v_ctrl_points: Array
    v_ctrl_points_w: Array
    current: float
    bem_solver: BEM2D
    trainable_params: tp.List[str] = ["v_ctrl_points_w", "current"]

    @classmethod
    def from_params(cls, **kwargs):

        # Defining grooving parameters
        if "nb_grooves" not in kwargs:
            nb_grooves = 5
        else:
            nb_grooves = kwargs["nb_grooves"]
        
        if "nb_ctrl_points" not in kwargs:
            nb_ctrl_points = 5
        else:
            nb_ctrl_points = kwargs["nb_ctrl_points"]

        # Defining control points and weights
        u_pos = np.tile(np.linspace(0, 1, nb_ctrl_points, endpoint=True)[None], (nb_grooves + 2, 1))
        v_pos = np.tile(np.linspace(0, 1, nb_grooves + 2, endpoint=True)[None], (nb_ctrl_points, 1)).T
        v_pos_w = np.zeros(((nb_grooves + 1) * (nb_ctrl_points - 1)))
        # v_pos_w = np.array([1., 0., 1., 0., # example with nb_grooves = 5 and nb_ctrl_points = 5
        #      0., 0., 0., 0.,
        #      0., 0., 0., 1.,
        #      0., 1., 0., 1.,
        #      0., 0., 0., 1.,
        ...
        control_points_uv_poloidal = []
        control_points_uv_toroidal = []

        for i in range(nb_grooves + 2):
            control_points_uv_poloidal += [np.stack((u_pos[i,:],v_pos[i,:]), axis=0).T]

        for i in range(nb_grooves + 1):
            control_points_uv_toroidal += [np.stack((control_points_uv_poloidal[i][0,:],control_points_uv_poloidal[i+1][0,:]), axis=1).T]
            control_points_uv_toroidal += [np.stack((control_points_uv_poloidal[i][-1,:],control_points_uv_poloidal[i+1][-1,:]), axis=1).T]

        control_points_poloidal_xy = [groove[:,::-1] for groove in control_points_uv_poloidal]
        control_points_toroidal_xy = [groove[:,::-1] for groove in control_points_uv_toroidal]

        # Defining BEM problem - main parameters
        if "current" not in kwargs:
            current = 1.0
        else:
            current = kwargs["current"]
        if "spline_degree" not in kwargs:
            spline_degree = 1
        else:
            spline_degree = kwargs["spline_degree"]
        
        if "nb_poloidal_elements" not in kwargs:
            nb_poloidal_elements = (nb_ctrl_points - 1) * 2
        else:
            nb_poloidal_elements = kwargs["nb_poloidal_elements"]

        if "nb_toroidal_elements" not in kwargs:
            nb_toroidal_elements = 2
        else:
            nb_toroidal_elements = kwargs["nb_toroidal_elements"]
        
        # Setting the problem geometry
        poloidal_lines_degrees = [spline_degree] * ((nb_grooves + 2))
        toroidal_lines_degrees = [1] * 2 * ((nb_grooves) + 1)
        num_poloidal_control_points = [nb_ctrl_points] * ((nb_grooves + 2))
        num_toroidal_control_points = [2] * 2 * ((nb_grooves + 1))

        # Calculating the boundary conditions: normal electric field (or current density) on boundaries and/or electric potential on boundaries
        initial_BC_normalElecField_toroidal_lines = [current * (nb_grooves + 1)] * (nb_grooves + 1)
        initial_BC_normalElecField_poloidal_lines = [0] * ((nb_grooves + 1))

        # Boundary conditions
        boundary_conditions = {
            # 'Dirichlet': {'south': [V*0 for V in initial_BC_toroidal_E]},
            'Neumann': {'east': initial_BC_normalElecField_poloidal_lines, 'west': initial_BC_normalElecField_poloidal_lines,'north': initial_BC_normalElecField_toroidal_lines, 'south': [-E for E in initial_BC_normalElecField_toroidal_lines]}
        }

        control_points_xy = (control_points_poloidal_xy, control_points_toroidal_xy)

        domain = Domain(poloidal_lines_degrees, toroidal_lines_degrees, num_poloidal_control_points,
            num_toroidal_control_points, boundary_conditions, nb_toroidal_elements, nb_poloidal_elements,control_points_xy)

        # Create an instance of the BEM class with the domain
        bem_solver = BEM2D(domain)

        # Solve the BEM system for all subdomains
        bem_solver.solve()

        # # Plotting the domain
        # bem_solver.plot()
            
        return cls(nb_grooves=nb_grooves, nb_ctrl_points=nb_ctrl_points, u_ctrl_points=u_pos, v_ctrl_points=v_pos, v_ctrl_points_w=v_pos_w, current=current, bem_solver=bem_solver)
    ...
    def update_params(self, **kwargs):
        ...
        self.update_v_ctrl_points()
        
        # Defining perturbed domain
        control_points_uv_poloidal_perturbed = []

        for i in range(1,self.nb_grooves + 1):
            control_points_uv_poloidal_perturbed += [np.stack((self.u_ctrl_points[i,:],self.v_ctrl_points[i,:]), axis=0).T]

        control_points_xy_poloidal_perturbed = [groove[:-1,::-1] for groove in control_points_uv_poloidal_perturbed]
            
        # Update the domain
        self.bem_solver.update_domain(self.current,control_points_xy_poloidal_perturbed)

        # Solve the BEM system for all subdomains
        self.bem_solver.solve()
    ...
    def get_j_2d(self, grid_points):
        """Compute j_2d on surface grid points from the groove parameters using the Boundary Element Method"""
        return self.bem_solver.post_process(grid_points)[1]

    def get_j_3d(self, grid_points):
        """Compute j_3d on surface grid points from the groove parameters using the Boundary Element Method"""
        raise NotImplementedError

    def update_v_ctrl_points(self,min_width=0.05):
        """
        Note that the first 'groove' is defined by v=0 and the last groove is defined by v=1.
        """  
        # Check the feasibility of epsilon
        num_elements = (self.v_ctrl_points.shape[0] - 1)
        if min_width * num_elements >= 1.0:
            raise ValueError("Minimum width is too large; it's impossible to satisfy the constraints.")

        # Reshape the width points
        v_ctrl_points_w_reshaped = self.v_ctrl_points_w.reshape(self.v_ctrl_points.shape[0] - 1, self.v_ctrl_points.shape[1] - 1)

        # Compute the softmax
        logits = v_ctrl_points_w_reshaped - np.max(v_ctrl_points_w_reshaped, axis=0)  # For numerical stability
        exp_logits = np.exp(logits)
        softmax_output = exp_logits / np.sum(exp_logits, axis=0)

        # Scale the softmax output so that the sum of all elements is (1 - epsilon * num_elements)
        scale_factor = 1.0 - min_width * num_elements
        scaled_softmax = softmax_output * scale_factor

        # Add epsilon to each element
        adjusted_widths = scaled_softmax + min_width

        # Normalize the adjusted widths to ensure they sum to 1
        normalized_widths = adjusted_widths / np.sum(adjusted_widths, axis=0)
        v_ctrl_points_except_north_boundary = np.concatenate((np.zeros_like(self.v_ctrl_points[:1,:-1]), np.cumsum(normalized_widths, axis=0)), axis=0)
        # Update v_ctrl_points with cumulative sum
        self.v_ctrl_points = np.concatenate((v_ctrl_points_except_north_boundary, v_ctrl_points_except_north_boundary[:,0].reshape(v_ctrl_points_except_north_boundary.shape[0],1)), axis=1)
    ...
    #     Note that the first 'groove' is defined by v=0 and the last groove is defined by v=1.
    #     """
    #     v_ctrl_points = np.cumsum(jax.nn.softmax(self.v_ctrl_points_w), axis=0)
    #     v_ctrl_points = np.concatenate((np.zeros_like(v_ctrl_points[:1]), v_ctrl_points), axis=0)
    #     return jax.vmap(np.interp, in_axes=(None, 0, 0))(u, self.u_ctrl_points, v_ctrl_points)
