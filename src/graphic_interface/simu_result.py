...
class Simu_result():
    ...
    def get_data_dic(self,surf_param=None):
        ...
        dic['cost j']='{:.3e}'.format(EM_dic['cost_J'])
        dic['max j']='{:.3e}'.format(EM_dic['max_j'])
        dic['EM cost']='{:.3e}'.format(EM_cost)
        dist_cost,dist_dic=self.dist.cost(S)
        dic['Distance']='{:.5e}'.format(dist_dic['min_distance'])
        perim_cost,perim_dic=self.perim.cost(S)
        dic['perimeter']='{:.3e}'.format(perim_dic['perimeter'])
        curv_cost,curv_dic=self.curv.cost(S)
        dic['Max curv']='{:.3e}'.format(curv_dic['max_curvature'])
