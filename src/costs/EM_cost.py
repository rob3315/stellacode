...
def EM_cost_dask(config,S,Sp):
    ...
    B_tilde=BT-np.einsum('tpq,t',LS[:2],[net_poloidal_current_Amperes ,net_toroidal_current_Amperes])
    LS_dagger_B_tilde=np.einsum('hpq,pq->h',LS_dagger_R,B_tilde)
    RHS=LS_dagger_B_tilde-lamb*Qj_inv_R@Qj[2:,:2]@[net_poloidal_current_Amperes ,net_toroidal_current_Amperes]
