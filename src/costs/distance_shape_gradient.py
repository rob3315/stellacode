...
class Distance_shape_gradient(Abstract_shape_gradient):
    ...
    def __init__(self, path_config_file=None, config=None):
        ...
        self.d_min_hard = float(
            config['optimization_parameters']['d_min_hard'])
        self.d_min_soft = float(
            config['optimization_parameters']['d_min_soft'])
        self.d_min_penalization = float(
            config['optimization_parameters']['d_min_penalization'])
        self.vf = np.vectorize(lambda x: f_non_linear(
    ...
    def cost(self, S):
        ...
        logging.info('min_distance {:5e} m, Distance cost : {:5e}'.format(
            np.min(dist_min), cost))
    ...
    def shape_gradient(self, S, theta_pertubation):
        ...
        def aux(x): return np.unravel_index(x, (Np, lu_plasma, lv_plasma))
        ...
        for i in range(lu):
            for j in range(lv):
                T_min[i, j, :] = self.rot_tensor[(
                    Np-indexes_fullNp[i, j]) % Np] @ T[indexes_fullNp[i, j], i, j, indexes_fullp[i, j], indexes_fullq[i, j], :]
                vgradf[i, j] = grad_f_non_linear(
                    self.d_min_hard, self.d_min_soft, self.d_min_penalization, dist_min[i, j])
                Y[i, j] = f_non_linear(
                    self.d_min_hard, self.d_min_soft, self.d_min_penalization, dist_min[i, j])
        ...
        grad_distance = self.Np*(np.einsum('oijl,ijl,ij->o', theta,
                                 X, S.dS/S.npts)+np.einsum('ij,oij->o', Y, dSdtheta/S.npts))
