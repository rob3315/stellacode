...
class Shape_gradient_dask():
    ...
    def compute_cost(self,paramS):
        ...
        dtype=np.float64
        self.dask_rot_tensor=self.dask_rot_tensor.map_blocks(cp.asarray,dtype=dtype)
        self.dask_matrixd_phi=self.dask_matrixd_phi.map_blocks(cp.asarray,dtype=dtype)
        dask_dpsi=dask_dpsi.map_blocks(cp.asarray,dtype=dtype)
        self.dask_eijk=self.dask_eijk.map_blocks(cp.asarray,dtype=dtype)
        dask_D=dask_D.map_blocks(cp.asarray,dtype=dtype)
        dask_DD=dask_DD.map_blocks(cp.asarray,dtype=dtype)
        ...
        LS_dask=contract('ijklmn,ojkw,ipz,wzjk,qnp,ijklm,qlm->olm',dask_T,self.dask_matrixd_phi,self.dask_rot_tensor,dask_dpsi,self.dask_eijk,dask_D,dask_normalp,optimize=True)
        LS=((mu_0/(4*np.pi))*LS_dask.compute()/(self.ntheta_coil*self.nzeta_coil)).get()
