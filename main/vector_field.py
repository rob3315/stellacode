...
def compute_Qj(matrixd_phi,dpsi,dS):
    """take only the segment whitout rotation of j"""
    lu,lv=dS.shape
    Qj=contract('oija,adij,kdij,pijk,ij->op',matrixd_phi,dpsi,dpsi,matrixd_phi,1/dS,optimize=True)/(lu*lv)
    return Qj
def compute_dQjdtheta(matrixd_phi,dpsi,dS,dtheta,dSdtheta):
    """take only the segment whitout rotation of j"""
    lu,lv=dS.shape
    #derivation with respect to dS
    dQj=contract('oija,adij,kdij,pijk,zij->zop',matrixd_phi,dpsi,dpsi,matrixd_phi,-dSdtheta/(dS*dS),optimize=True)/(lu*lv)
    ...
    dQj+=contract('oija,zijad,kdij,pijk,ij->zop',matrixd_phi,dtheta,dpsi,matrixd_phi,1/dS,optimize=True)/(lu*lv) \
        + contract('oija,adij,zijkd,pijk,ij->zop',matrixd_phi,dpsi,dtheta,matrixd_phi,1/dS,optimize=True)/(lu*lv)
    return dQj
...
def compute_LS_old(T,j,dS,normalp):
    ...
    LS=contract('ijklmn,iojkp,qnp,jk,ijklm,qlm->olm',T,j,eijk,dS,D,normalp,optimize=True)
...
def compute_LS(T,matrixd_phi,dpsi,rot_tensor,normalp):
    ...
    eijk[0, 2, 1] = eijk[2, 1, 0] = eijk[1, 0, 2] = -1
    #the terrible formula...
    LS=contract('ijklmn,ojkw,ipz,wzjk,qnp,ijklm,qlm->olm',T,matrixd_phi,rot_tensor,dpsi,eijk,D,normalp,optimize=True)
    return (mu_0/(4*np.pi))*LS/(lu1*lv1)
...
def compute_dLS(T,matrixd_phi,dpsi,rot_tensor,normalp,theta,dtildetheta):
    ...
    dLS+=3*contract('ibc,ajkc,ijklmb,ijklmn,ojkw,ipz,wzjk,qnp,ijklm,qlm->aolm',rot_tensor,theta,T,T,matrixd_phi,rot_tensor,dpsi,eijk,DD,normalp,optimize=True)/(lu1*lv1)
